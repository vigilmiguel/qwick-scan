...
class barcode_spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url='https://www.barcodelookup.com/' + self.targetBarcode, callback=self.parse, meta={'index':0, 'barcode': self.targetBarcode, 'barHash': barHashTable})
        #yield scrapy.Request(url='https://www.google.com/search?psb=1&tbm=shop&q=' + self.targetBarcode, callback=self.parse, meta={'index':1, 'barcode': self.targetBarcode, 'barHash': barHashTable})
        yield scrapy.Request(url='https://brickseek.com/products/?search=' + self.targetBarcode, callback=self.parse, meta={'index':2, 'barcode': self.targetBarcode, 'barHash': barHashTable})
    ...
    def parse(self, response):
        ...
        if(response.meta['index'] == 0):
           ...
           yield scrapy.Request(url='https://www.google.com/search?psb=1&tbm=shop&q=' + response.meta['barHash'].get(self.targetBarcode)[0], callback=self.parse0, meta={'index':1, 'barcode': self.targetBarcode, 'barHash': response.meta['barHash']})
